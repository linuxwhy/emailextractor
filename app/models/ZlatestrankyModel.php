...
class ZlatestrankyModel extends Nette\Object{
	...
	static function getFluent(){
		return dibi::select("*")
				->from("link_zlatestranky")
				->join("zlatestranky_email")->on("(link_id = link_zlatestranky.id)")
				->where('zlatestranky_email.checked = 1');
	}
	
	static function getNoCheckPages($limit = 10){
		return dibi::fetchAll("SELECT link,id,parent_id FROM [link_zlatestranky] WHERE 	check_date IS NULL LIMIT ".$limit);
	}
...
//				
//				if(isset($link[0]) AND $link[0] == '/'){
//					$link = 'www.zlatestranky.sk'.$link;
//				}

				$arr_link = parse_url($link);
				
				if(!isset($arr_link['host'])){
					$arr_link['host'] = 'www.zlatestranky.sk';
				}
...
//					';
				//ak adresa obsahuje www.zlatestranky.sk
				$link_to_return = $arr_link['host'].$arr_link['path'];
				if( strstr($link_to_return,'www.zlatestranky.sk/firmy/')){
...
//		print_r($meta);
		...
		if( !dibi::fetchSingle("SELECT 1 FROM [link_zlatestranky] WHERE link = %s",$link) ){
...
//				   $text = $element->innertext; 
	...
	static function add($values){
		dibi::query("INSERT INTO [link_zlatestranky] ", $values);
	}
...
//		$this->pr($content);
	...
	static function update($values, $id){
		dibi::query("UPDATE [link_zlatestranky] SET",$values, "WHERE id = %i",$id);
	}
	
	static function getLinksForCheck($limit = 10){
		return dibi::fetchAll("SELECT * FROM [link_zlatestranky] WHERE checked_for_email = 0 LIMIT ".$limit);
	}
	...
	static function getCompanyInfo( $link ){
		...
		foreach($links as $k=>$l){
			if( !strstr($l,'www.zlatestranky.sk') )
				unset($links[$k]);
		}
...
//	
	...
	static function addEmail($id_link, $email){
		...
		dibi::query("INSERT INTO [zlatestranky_email]", array('email'=>$email,'link_id'=>$id_link));
	}
